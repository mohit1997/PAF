...
def taylor(inp, k, trainable=False, name="activation", scale=None, decay=0.5, is_train=False):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if len(X.get_shape()) == 4:
            ...
            if is_train:
                max_tensor = update_op(pop_max, max_tensor, m, n, decay)
            else:
                max_tensor = m*pop_max + n 
            Xin = X/max_tensor
