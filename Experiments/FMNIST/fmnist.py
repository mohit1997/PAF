...
def deepnn(x,n_classes):
    ...
    h = tf.layers.conv2d(h, filters=64, kernel_size=(3, 3), padding='SAME')
    h = tf.nn.relu(h)
    h = tf.layers.batch_normalization(h, training=is_train)
    ...
    h = tf.layers.conv2d(h, filters=128, kernel_size=(3, 3), padding='SAME')
    h = tf.nn.relu(h)
    h = tf.layers.batch_normalization(h, training=is_train)
    # h = tf.contrib.nn.alpha_dropout(h, keep_prob=keep_prob)
    h = tf.layers.conv2d(h, filters=128, kernel_size=(3, 3), padding='SAME')
    ...
    h = tf.contrib.layers.fully_connected(h, 512)
    h = tf.layers.batch_normalization(h, training=is_train)
    h = tf.nn.relu(h)

    # h = tf.contrib.layers.fully_connected(h, 512, weights_regularizer=tf.contrib.layers.l2_regularizer(scale=0.005))
    h = tf.contrib.layers.fully_connected(h, 512)
    h = tf.layers.batch_normalization(h, training=is_train)
    h = tf.nn.relu(h)    
