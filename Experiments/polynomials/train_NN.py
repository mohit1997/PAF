...
def deepnn(x, n_classes):
    ...
    h = tf.contrib.layers.fully_connected(x, 512, activation_fn=None, weights_regularizer=tf.contrib.layers.l1_regularizer(scale=0.0005))
...
def main(_):
    # Import data
    X, Y = get_polynomial(num_features=num_features, degree=3, samples=100000)
