...
def deepnn(x, n_classes):
    ...
    h = tf.contrib.layers.fully_connected(x, 512, activation_fn=None, weights_regularizer=tf.contrib.layers.l1_regularizer(scale=0.005))
    ...
    h = tf.contrib.layers.fully_connected(h, 512, activation_fn=None)
    # h = tf.keras.layers.Dense(1000)(x)
...
def main(_):
    # Import data
    X, Y = get_polynomial(num_features=num_features, num_terms=10, degree=3, samples=100000)
    ...
    with tf.name_scope('Adam_optimizer'):
        ...
        with tf.control_dependencies(update_ops):
            ...
            train_step = tf.train.AdamOptimizer(learning_rate).minimize(w_loss+mean_squared_error)
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            if i % 10 == 0:
                train_accuracy, regloss = sess.run([mean_squared_error, w_loss], feed_dict={x: x_train, y_: y_train, is_train: False, keep_prob: 0.8})
