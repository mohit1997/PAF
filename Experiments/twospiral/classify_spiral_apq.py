...
def hidden_RAMO(is_train):
    ...
    # h1 = taylor(tf.matmul(x, W1) + b1, k=3, is_train=is_train, name='Ac/1')
    # h3 = taylor(tf.matmul(h1, W2) + b2, k=3, is_train=is_train, name='Ac/2')
    print("Using APQ")
    h1 = apq(tf.matmul(x, W1) + b1, is_train=is_train, name='Ac/1')
    h3 = apq(tf.matmul(h1, W2) + b2, is_train=is_train, name='Ac/2')
