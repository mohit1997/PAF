...
def lr(iteration, low=0.08, high=0.8, steps=max_iter, midstep=4000):
...
def resnet_module(data, K, stride, is_train, red=False):
	...
	h = tf.layers.batch_normalization(h, training=is_train)
	h = tf.nn.relu(h)
	h = tf.layers.conv2d(h, filters=int(K*0.25), kernel_size=(3, 3), strides=stride, padding='SAME', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.005))

	h = tf.layers.batch_normalization(h, training=is_train)
	h = tf.nn.relu(h)
	h = tf.layers.conv2d(h, filters=int(K), kernel_size=(1, 1), padding='SAME', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.005))
...
def resnet(x, is_train, n_classes, stages=(3, 3, 3), filters=(32, 32, 32, 32)):
	orig = h = x

	h = tf.layers.conv2d(h, filters=int(filters[0]), kernel_size=(5, 5), padding='SAME', kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.005))
	h = tf.layers.batch_normalization(h, training=is_train)
	h = tf.nn.relu(h)

	h = tf.keras.layers.MaxPool2D((3, 3), strides=(2, 2), padding='same')(h)

	h = tf.keras.layers.MaxPool2D((3, 3), strides=(2, 2))(h)
...
def main():
	...
	with tf.name_scope('Loss'):
		...
		reg_loss = 10.0 * tf.losses.get_regularization_loss()
	...
	with tf.Session() as sess:
		...
		for i in range(max_iter):
			batch = cifar.next_train_batch(512, augment=False)
			if i % 100 == 0:
