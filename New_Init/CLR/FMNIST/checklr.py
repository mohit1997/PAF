...
def main(_):
    ...
    with tf.name_scope('Adam_optimizer'):
        ...
        with tf.control_dependencies(update_ops):
            if clip == True:
                ...
                train_step = tf.train.MomentumOptimizer(learning_rate, momentum=0.9, use_nesterov=True).minimize(cross_entropy + reg_loss)
    ...
    with tf.Session() as sess:
        ...
        for i in range(total_iterations):
            ...
            train_accuracy = cross_entropy.eval(feed_dict={x: batch[0], y_: batch[1], is_train: True, keep_prob: 0.8})
            acc.append(train_accuracy)
            train_step.run(feed_dict={x: batch[0], y_: batch[1], is_train: True, learning_rate: lr(i, steps=total_iterations), keep_prob: 0.8})
            print(i, train_accuracy)
            np.save('list_checklr', np.array(acc))
