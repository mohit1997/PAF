...
def deepnn(x,n_classes):
    ...
    # h = gaussian_noise_layer(h, is_train, 0.01)
...
def main(_):
    ...
    with tf.name_scope('Loss'):
        ...
        regl2_loss =  1.0 * tf.add_n([tf.reduce_sum(tf.nn.l2_loss(tf.cast(v, tf.float32))) for v in temp])
        reg_loss = regl2_loss
        w_loss = 10.0 * w_loss
    ...
    with tf.name_scope('Adam_optimizer'):
        ...
        with tf.control_dependencies(update_ops):
            ...
            reg_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(w_loss+regl2_loss)
