...
def taylor(inp, k, is_train=False, name="activation", scale=None, decay=0.99, reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if len(X.get_shape()) == 4:
            ...
            # pop_mean = tf.get_variable(initializer=tf.zeros([1, 1, 1, C, k]), trainable = False, name="means")
            # pop_var = tf.get_variable(initializer=tf.ones([1, 1, 1, C, k]), trainable = False, name="variances")
            ...
            bias = bias_variable(name="bias", shape=[C])
            print(bias)
        ...
        if len(X.get_shape()) == 3:
            ...
            # pop_mean = tf.get_variable(initializer=tf.zeros([1, 1, C, k]), trainable = False, name="means")
            # pop_var = tf.get_variable(initializer=tf.ones([1, 1, C, k]), trainable = False, name="variances")
            # # max_tensor = tf.cond(is_train, lambda: update_op(pop_max, max_tensor, m, n, decay), lambda: m*max_tensor + n)
            # if type(is_train) is not bool:
            #     Xin = tf.cond(is_train, lambda: update_op(X, pop_mean, pop_var, batch_mean, batch_var, decay), lambda: tf.nn.batch_normalization(X, pop_mean, pop_var, offset=None, scale=None, variance_epsilon=1e-8))
            # else:
        ...
        if len(X.get_shape()) == 2:
            ...
            # pop_mean = tf.get_variable(initializer=tf.zeros([1, N, k]), trainable = False, name="means")
            # pop_var = tf.get_variable(initializer=tf.ones([1, N, k]), trainable = False, name="variances")
            # # max_tensor = tf.cond(is_train, lambda: update_op(pop_max, max_tensor, m, n, decay), lambda: m*max_tensor + n)
            # if type(is_train) is not bool:
            #     Xin = tf.cond(is_train, lambda: update_op(X, pop_mean, pop_var, batch_mean, batch_var, decay), lambda: tf.nn.batch_normalization(X, pop_mean, pop_var, offset=None, scale=None, variance_epsilon=1e-8))
            # else:
            ...
            bias = bias_variable(name="bias", shape=[1])
            conv = tf.reduce_sum(kernel * Xin, axis=-1) + bias
            return(conv)