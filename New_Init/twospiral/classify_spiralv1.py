...
from taylor_fast import taylor
import sys
...
# plt.scatter(x_train[:, 0], x_train[:, 1], c=np.float32(2*y_train[:, 0])/10)
# # plt.scatter(x_test[:, 0], x_test[:, 1], c=np.float32(2*y_test[:, 0])/10)
...
def hidden_taylor(is_train):
    ...
    with tf.variable_scope("model"):
        ...
        W4 = create_weight_variable('Weights4', [val, n_classes])
        b4 = create_bias_variable('Bias4', [n_classes])
    h1 = taylor(tf.matmul(x, W1) + b1, k=5)
    h2 = taylor(tf.matmul(h1, W2) + b2, k=4)
    h3 = taylor(tf.matmul(h2, W3) + b3, k=3)
    t = (tf.matmul(h3, W4) + b4)
    # t = activation_function_taylor(t, k=4, is_train=is_train)
    return t, t
...
def hidden_RAMO(is_train):
    ...
    with tf.variable_scope("model"):
        ...
        W4 = create_weight_variable('Weights4', [val, n_classes])
        b4 = create_bias_variable('Bias4', [n_classes])
    h1 = RAMO(tf.matmul(x, W1) + b1, k1=8, k2=8)
    h3 = RAMO(tf.matmul(h1, W2) + b2, k1=8, k2=8)
    # h1 = tf.nn.relu(tf.matmul(x, W1) + b1)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for batch_x, batch_y in iterate_minibatches(x_train, y_train, batchsize=batch_size, shuffle=True):
            _, c, summary, batch_acc = sess.run([train_op, cost, merged_summary_op, acc], feed_dict={x: batch_x, y: batch_y, is_train: True, learning_rate: 1e-3})
