...
# plt.plot(taylor_bn, 'k', label=('SLAF (max= {:.2f}%)'.format(taylor_bn_max*100)))
# plt.plot(relu, 'b', label=('ReLU (max= {:.2f}%)'.format(relu_max*100)))
# plt.plot(taylor_max, 'g', label=('Mc Poly_MS (max= {:.2f}%)'.format(taylor_max_max*100)))
# plt.plot(taylor_bn, 'k', label=('Mc Poly_BN (max= {:.2f}%)'.format(taylor_bn_max*100)))
...
plt.savefig('Accuracy_Curve_BTPII.png')