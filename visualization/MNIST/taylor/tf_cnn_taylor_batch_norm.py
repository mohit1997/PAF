...
def deepnn(x, is_train):
  ...
  with tf.name_scope('conv1'):
    ...
    temp_c1 = (conv2d(x_image, W_conv1) + b_conv1) #UC
    # _, _,out,_, _ = non_linear_Ac(temp_1, input_size = 25088, k1 = 4, k2 = 4, output_size = 25088) #UC
    # h_conv1, w1, b1 = RAMO(temp_c1, k1=4, k2=4, scale=None) #UC
    # h_conv1 = temp_c1
    h_conv1 = taylor(temp_c1, k=4, is_train=is_train, name="Ac/1")
  ...
  with tf.name_scope('conv2'):
    ...
    # h_conv2 = temp_c2
    h_conv2 = taylor(temp_c2, k=1, is_train=is_train, name="Ac/2")
  ...
  with tf.name_scope('fc1'):
    ...
    # h_fc1 = h_fc
    h_fc1 = taylor(h_fc, k=1, is_train=is_train, name="Ac/3")
...
def main(_):
  ...
  y_conv, keep_prob, penul = deepnn(x, is_train=is_train)
  ...
  with tf.Session() as sess:
    ...
    for i in range(5000):
      ...
      if i % 100 == 0:
        ...
        x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0, is_train: False})
        print(a)
        acc.append(a)

      # print(batch[0][0])
      train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0, learning_rate: lr(i), is_train: True})
    ...
    vis_tensor = penul.eval(feed_dict={
        x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0, is_train: False})
    np.save('vis_tensor', vis_tensor)
    np.save('labels', mnist.test.labels)
