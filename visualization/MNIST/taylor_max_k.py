...
def taylor(inp, k, is_train, name="activation", scale=None, decay=0.99):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if len(X.get_shape()) == 4:
            ...
            pop_max = tf.Variable(tf.ones([1, 1, 1, C, k]), trainable = False)
        ...
        if len(X.get_shape()) == 3:
            ...
            pop_max = tf.Variable(tf.ones([1, 1, C, k]), trainable = False)
            max_tensor = tf.cond(is_train, lambda: update_op(pop_max, max_tensor, m, n, decay), lambda: m*pop_max + n)
            Xin = X/max_tensor
        ...
        if len(X.get_shape()) == 2:
            ...
            pop_max = tf.Variable(tf.ones([1, N, k]), trainable = False)
