...
def deepnn(x, is_train):
  ...
  with tf.name_scope('conv1'):
    ...
    # h_conv1 = taylor(temp_1, k=4, is_train=is_train, name="activation1")
    h_conv1 = apq(temp_1, name="activation1")
  ...
  with tf.name_scope('conv2'):
    ...
    # h_conv2 = taylor(temp_c2, k=4, is_train=is_train, name="activation2")
    h_conv2 = apq(temp_c2, name="activation2")
  ...
  with tf.name_scope('fc1'):
    ...
    # h_fc1 = taylor(h_fc, k=4, is_train=is_train, name="activation3")
    h_fc1 = apq(h_fc, name="activation3")
...
def main(_):
  ...
  with tf.name_scope('adam_optimizer'):
    t1 = tf.train.AdamOptimizer(learning_rate).minimize(cross_entropy, var_list=[v for v in tf.trainable_variables() if not include_activation(v.name)])
    t2 = tf.train.MomentumOptimizer(1e-4, momentum=0.9).minimize(cross_entropy, var_list=[v for v in tf.trainable_variables() if include_activation(v.name)])
    # t3 = tf.train.GradientDescentOptimizer(learning_rate).minimize(l2_loss)
  ...
  with tf.Session() as sess:
    ...
    # a = (sess.run([w1,b1,w2], feed_dict={x: mnist.test.images[0:5000, :], y_: mnist.test.labels, keep_prob: 1.0}))
    # w1 = np.array(a[0])
    # b1 = np.array(a[1])
    # w2 = np.array(a[2])
    # b2 = np.array(a[3])
    ...
    # np.save('w3', w3)
    # np.save('b3', b3)
    # np.save('testing_score_SLAF', np.array(acc))
    np.save('testing_score_apq', np.array(acc))
