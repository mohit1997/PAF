...
from taylor_gradient_clipping import taylor
...
def deepnn(x):
  ...
  with tf.name_scope('conv1'):
    ...
    h_conv1 = taylor(temp_1, k=4, name="Ac/1")
  ...
  with tf.name_scope('conv2'):
    ...
    h_conv2 = taylor(temp_c2, k=4, name="Ac/2")
  ...
  with tf.name_scope('fc1'):
    ...
    h_fc1 = taylor(h_fc, k=4, name="Ac/3")
...
def main(_):
  ...
  with tf.name_scope('adam_optimizer'):
    ...
    train_step1 = optimizer.apply_gradients(zip(gradients, variables))
    train_step2 = tf.train.GradientDescentOptimizer(learning_rate).minimize(l2loss) 
  ...
  with tf.Session() as sess:
    ...
    for i in range(25000):
      ...
      # print(batch[0][0])
      train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5, learning_rate: lr(i)})
    ...
    # np.save('w3', w3)
    # np.save('b3', b3)
    np.save('testing_score_taylor_gradient_clipping', np.array(acc))
