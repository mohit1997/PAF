...
def model():
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        conv = taylor(conv, k=2, is_train=is_train, name="Ac/1")
        # conv = tf.nn.relu(conv)
        # tf.contrib.layers.batch_norm(conv, is_training=is_train)
        ...
        conv = taylor(conv, k=2, is_train=is_train, name="Ac/2")
        # conv = tf.nn.relu(conv)
        # tf.contrib.layers.batch_norm(conv, is_training=is_train)
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        # conv = taylor(conv, k=4, is_train=is_train, name="Ac/3")
        # conv = tf.nn.relu(conv)
        tf.contrib.layers.batch_norm(conv, is_training=is_train)
        # pool = tf.layers.max_pooling2d(conv, pool_size=[2, 2], strides=2, padding='SAME')
        pool = tf.layers.average_pooling2d(conv, pool_size=[2, 2], strides=2, padding='SAME')
