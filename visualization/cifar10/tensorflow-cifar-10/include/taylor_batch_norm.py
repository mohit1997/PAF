...
def taylor(inp, k, is_train=False, name="activation", scale=None, decay=0.99, reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if len(X.get_shape()) == 4:
            ...
            # attention = tf.nn.softmax(create_variables("activation_attention", [1, k]), axis=-1)
            kernel = create_variables("activation_weights", [C, k], scale=scale)
            coeff = tf.identity(attention * kernel * tf.pow(2.0, tf.range(k, 0, delta=-1, dtype=tf.float32)), name="activation_coeff")

            kernel = kernel + tf.concat([tf.ones([C, 1]), tf.zeros([C, k-1])], axis=-1)

            bias = bias_variable(name="activation_bias", shape=[C])
            print(bias)
            conv = tf.math.add(tf.reduce_sum(attention * kernel * Xin, axis=-1), bias, name="activation_output")
            return(conv)
        ...
        if len(X.get_shape()) == 2:
            ...
            attention = tf.nn.sigmoid(create_variables("activation_attention", [1, k]))
            # attention = tf.nn.softmax(create_variables("activation_attention", [1, k]), axis=-1)
            kernel = create_variables("activation_weights", [1, k])
            coeff = tf.identity(attention * kernel * tf.pow(2.0, tf.range(k, 0, delta=-1, dtype=tf.float32)), name="activation_coeff")
            kernel = kernel + tf.concat([tf.ones([1, 1]), tf.zeros([1, k-1])], axis=-1)
